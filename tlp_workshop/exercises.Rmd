...
```{r}
#paletteer is package that has hundreds of differe color pallets that can easily be used with ggplot
library(paletteer) 
paletteer::palettes_d_names # to see list of all palette names for discrete (categorical) vaariables
paletteer::palettes_c_names # to see list of all palette names for continuous vaariables
...
# also shortcut for log transform:
...
# but the size for 4 cyl looks way too small compared to rest

ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy, colour = class, size = cyl)) + 
  scale_colour_brewer(type = 'qual') +
  scale_radius(breaks = c(4,5,6,8))
# better, but with many breaks can get our of hand fast

ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy, colour = class, size = cyl)) + 
  scale_colour_brewer(type = 'qual') +
  scale_size_area(breaks = c(4,5,6,8))
# ok, but not much difference from first to last bubble size
...
ggplot(mpg) + 
  ...
  scale_color_paletteer_c("gameofthrones::lannister")
```
...
# can free just one axis if wanted
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy)) + 
...
ggplot(mpg) + 
  ...
  facet_wrap(~ drv + year)
```
Maybe not as intuitive to look at but it is possible if you want to do it this way.
...
and in the coord. But zooming with scale removes data outside your set limits. You usually want the latter as it avoids changing the plotted data
...
world <- sf::st_as_sf(maps::map('world', plot = TRUE, fill = TRUE))
world <- sf::st_wrap_dateline(world, 
                              options = c("WRAPDATELINE=YES", "DATELINEOFFSET=10"),
                              quiet = TRUE)  #DOES NOT WORK
# Plot code
ggplot(world) + 
  geom_sf() + 
  coord_sf(crs = "+proj=robin")

# from coord_sf() help file:
library(maps)
...
world2 <- sf::st_transform(
  world1,
  "+proj=laea +y_0=0 +lon_0=-90 +lat_0=0 +ellps=WGS84 +no_defs"
)
...
#library(extrafont) # add additional fonts to choose from
#font_import(prompt=FALSE)
loadfonts(device = "win")
windowsFonts() #show what fonts are available
...
# in theme(), text = element_text('Avenir Next Condensed') did not work, error says font family not found in Windows font database
...
# Webinar Part II (https://www.youtube.com/watch?v=0m4yywqNPVY&t=7s)
## Extensions
...
p_all & theme(legend.position = 'none') # the '&' symbol allows you to add ggplot2 elements (theme(), geom_x()) to all subplots in patchwork. Here we turn off the legend to all subplots in one single line
p_all <- p_all & theme(legend.position = 'none')

# Can add labels to each plot
...
p_tot <- p + p + p
p_tot + plot_layout(widths = c(1,2,1))

# can also change widths using units of measurements for specific sizes, unit() is from grid
p_tot + plot_layout(widths = unit(c(5,1,1), c('cm', 'null', 'null')))

# now first plot will have fixed width of 5 cm
```

* * *

The `&` operator can be used with any type of ggplot2 object, not just themes. It usually is because you want to modify everything at once in themes(), but can be used with others.
...
#But with this, the labels are overlapping other points that are not labelled becasue the layer doenst know anything about the other points and doesnt know not to overlap them
...
#Can also put multiple on same graph

ggplot(mtcars, aes(x = disp, y = mpg)) +
  geom_point() + 
  geom_mark_hull(aes(fill = factor(gear), label= gear), concavity = 5)
```

Also reccommends to look into **gghighlight** - allows you to draw attention to aspects other than with text 
...
Can also use ggforce() to accomplish same goal
